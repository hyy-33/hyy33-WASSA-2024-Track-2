...
class Regressor(nn.Module):
    ...
    def __init__(self, drop_rate=0.1):
        ...
        self.bert = BertModel.from_pretrained('./240528')  # loading pretrained model from local path
        self.regressor = nn.Sequential(
            nn.Dropout(drop_rate),
            nn.Linear(D_in, D_out))

    def forward(self, input_ids, attention_mask):
        outputs = self.bert(input_ids, attention_mask)  # outputs.last_hidden_layer, outputs.pooler_output
...
epochs = 50
res_file = predict_target + "-bert-reg-fgm-mse epoch=" + str(epochs) + " lr=" + str(learning_rate) + " bs=" + str(bs) + ".txt"
...
for epoch in range(epochs):
    ...
    if (epoch + 1) % 2 == 0:  # evaluate
        ...
        print(eval_res)
        file.write(eval_res + "\n")
        torch.save(model, './bert-reg-fgm-mse/' + str(pear.item()) + "-" + str(epoch) + '.pth')
